...
\n**.help** - Irá te mandar a lista de comandos.
...
**.corona** - mostra os valores dos casos de corona.
**.insta** - exibe o perfil da pessoa do instagram.
**.m** - faz contas de matematica.
**.tempo** - mostra como está o tempo/clima.
...
**.userinfo** \`@usuário\` - Irá mostrar as informações do usuário mencionado.`)
               ...
               const ajudaX = new Discord.MessageEmbed()
               .setTitle(`🔊 Audios`)
               .setDescription(`*Comandos em que o jeremir entra na call e começa a falar!* \n
\n**.haha** - jeremir fala ta tao engraçado né.
**.apaixonar** - Tem certeza? vai se apaixonar. 
**.bonitao** - 'Bonitao na foto em chefe'. 
**.cadu** - 'cade o cadu joao frango?'.
**.maicon** - 'salve dinho, salve negao'.  
**.poha** - 'TA POHA jukes'. 
**.tiringa** - 'Tu me chamou pra perguntar isso?'. 
**.tliste** - fica tliste não`)
...
**.otaku** - +18. `)
               ...
               message.author.send(categoria).then(msg=> {
                   msg.react("👮").then(r => {
                       ...
                       const vozX = (reaction, user) => reaction.emoji.name === '🔊' && user.id === message.author.id;
                       ...
                       const vozX2 = msg.createReactionCollector(vozX, { time: 600000 });
                       ...
                       vozX2.on('collect', r=> {
                        message.author.send(ajudaX).catch(error => message.reply(`ops! Seu privado está desativado e não consigo enviar a ajuda. :worried:`).then(msg => msg.delete(8000)));
                       })
